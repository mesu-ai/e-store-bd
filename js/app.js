...
const showProducts = () => {
  ...
  for (const product of allProducts) {
    ...
    <div class="card h-100 single-product card-style" style="background-color:rgb(200, 213, 221)">
      ...
      <button onclick="productDetails(${product.id - 1})" id="details-btn" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Details</button>
...
const productDetails = productNumber => {

  const products = loadProducts();
  const product = products[productNumber];

  const detailsShow = document.getElementById('product-details');
  detailsShow.textContent = '';
  const div = document.createElement('div');
  div.innerHTML = `
  
          <div class="d-flex justify-content-center">
              <img class="productDetails-image" src=" ${product.image}" alt="">
          </div>
          <div class="text-center">
              <p class="fs-5"><span class="fw-bold">Title:</span>  ${product.title}</p>
              <h3 class="">Price: $<span class="fw-bold text-danger"> ${product.price}</span>  </h3>
          ...
          <div class="modal-footer d-flex justify-content-center ">
            ...
            <button onclick="addToCart(${product.id},${product.price})" type="button" class="btn btn-primary">Add to Cart</button>
